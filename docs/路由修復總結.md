...
/gallery/reader/<path>       -> Gallery閱讀器

# API 路由
/api/config                -> 獲取配置
/api/manga/list            -> 獲取漫畫列表
/api/gallery/list            -> 獲取Gallery列表
/api/manga/<path>          -> 獲取漫畫詳情
/api/gallery/<path>          -> 獲取Gallery詳情
/api/manga/chapter/<path>  -> 獲取漫畫章節圖片
/api/gallery/chapter/<path>  -> 獲取Gallery章節圖片

# 資源路由
/manga/image/<path>        -> 提供漫畫圖片
/gallery/image/<path>        -> 提供Gallery圖片
```
...
2. **點擊章節進入閱讀器**
   ```
   URL: http://localhost:5000/gallery/reader/作者名/作品集/
   ```

3. **閱讀器請求章節數據**
   ```
   API: /api/gallery/chapter/作者名/作品集
   返回: {"images": [...], "navigation": {...}}
   ```

4. **載入圖片**
   ```
   圖片: /gallery/image/作者名/作品集/001.jpg
   ```

## ✨ 已實現功能

### 1. 路由分離
- ✅ 漫畫和 Gallery 使用不同的 URL 路徑
- ✅ 清晰的 RESTful API 設計
- ✅ 獨立的資源路由
