...
| **主頁載入** | 首次載入 6 個項目，滾動到底部繼續載入下 6 個 |
| **圖卡顯示** | 封面圖片 + 下方顯示圖片數量 |
| **Reader** | 初始載入前 3 張圖片 + 總數量，其餘用空佔位符，滾動到佔位符再載入下 3 張（懶加載） |
...
#### 1.1 漫畫 Service (`modules/manga/service.py`)
- [x] `get_manga_list()` - 預設 `per_page=6`
- [x] `get_manga_list()` - 增加 `include_chapters=True` 參數，取得章節列表
...
#### 2.1 漫畫主頁 JS (`static/js/manga/index.js`)
| 項目 | 修改內容 |
|------|----------|
| 載入邏輯 | `per_page=6`，無限滾動載入 |
| 圖卡顯示 | 顯示章節列表（帶滾動） |
| 章節列表 | 固定高度，顯示 5 個章節，超過滾動 |

#### 2.2 漫畫 CSS (`static/css/manga.css`)
| 項目 | 修改內容 |
|------|----------|
...
#### 2.5 漫畫 Reader JS (新建 `static/js/manga/reader.js`)
| 項目 | 修改內容 |
|------|----------|
| 載入邏輯 | 一次載入所有圖片 |
| 顯示方式 | 順序顯示 |
...
|------|----------|
| 初始載入 | 前 3 張圖片 + 空佔位符 |
| 懶加載 | Intersection Observer 偵測，每次載入 3 張 |
| 總數顯示 | 顯示總圖片數量 |
...
```
[頁面載入] 
    → API: GET /manga/api/list?page=1&per_page=6&skip_chapters=false
    → 返回: { mangas: [{name, path, chapters: [...], cover_image}], total, page, total_pages }
    → 顯示 6 個漫畫卡片（含章節列表）

[滾動到底部] 
    → API: GET /manga/api/list?page=2&per_page=6&skip_chapters=false
    → 追加 6 個漫畫卡片
```
...
```
[頁面載入]
    → API: GET /manga/api/chapter/{chapter_path}
