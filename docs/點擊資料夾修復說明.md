...
在實現了列表頁面的分頁載入優化後（`skip_chapters=true`），點擊漫畫或 PIXIV 資料夾無法進入閱讀器。
...
添加別名路由以支持 `/api/manga/detail/<path>` 和 `/api/pixiv/detail/<path>`：
...
@app.route('/api/pixiv/detail/<path:manga_path>')
def get_pixiv_detail_alt(manga_path):
    """API：獲取PIXIV詳情（別名路由）"""
    return get_pixiv_detail(manga_path)
```
...
### PIXIV 相關
- `GET /api/pixiv/list` - 獲取 PIXIV 列表（分頁，可選 skip_chapters）
- `GET /api/pixiv/<path>` - 獲取單個 PIXIV 作品詳情（含章節列表）
- `GET /api/pixiv/detail/<path>` - 同上（別名路由）
- `GET /api/pixiv/chapter/<path>` - 獲取章節圖片列表
- `GET /pixiv/image/<path>` - 獲取圖片文件
