# Gallery 功能優化說明

## 📋 需求

### Gallery 列表頁面
- ✅ 初始只載入 6 個資料夾
- ✅ 滾輪捲動時才載入更多
- ✅ 顯示圖片張數（不實際載入圖片）
- ✅ 資料夾內無子資料夾（直接是圖片）

### Gallery 閱讀器
- ✅ 依序 lazy loading 全部圖片
- ✅ 每張載入完成立即顯示
- ✅ 自動繼續載入下一張
- ✅ 避免一次全部等待導致 timeout

## ✨ 實現的優化

### 1. 後端優化 (app.py)

#### 新增快速圖片計數方法
```python
def _count_images_fast(self, manga_path):
    """快速統計圖片數量（用於 Gallery）"""
    try:
        count = sum(
            1 for f in manga_path.iterdir() 
            if f.is_file() and f.suffix.lower() in IMAGE_EXTENSIONS
        )
        return count
    except:
        return 0
```

#### 優化列表載入邏輯
```python
if subdirs:
    # 有子資料夾（漫畫模式）
    chapter_count = len(subdirs)
else:
    # 沒有子資料夾（Gallery 模式）- 直接統計圖片數
    chapter_count = self._count_images_fast(manga_dir)
```

#### Gallery API 默認每頁 6 個
```python
@app.route('/api/gallery/list')
def get_gallery_list():
    page = request.args.get('page', 1, type=int)
    per_page = request.args.get('per_page', 6, type=int)  # 默認 6 個
    skip_chapters = request.args.get('skip_chapters', 'true').lower() == 'true'
    
    result = gallery_reader.get_manga_list(page=page, per_page=per_page, skip_chapters=skip_chapters)
    return jsonify(result)
```

#### 章節名稱優化
```python
# Gallery 模式：使用資料夾名稱而不是「單行本」
chapters.append({
    'name': manga_path.name,  # 資料夾名稱
    'path': formatPathForUrl(manga_path.relative_to(self.root_path)),
    'image_count': len(images)
})
```

### 2. 前端優化 (index.html)

#### 顯示圖片張數
```javascript
if (currentCategory === 'gallery') {
    // Gallery：顯示圖片張數
    chaptersHtml = `
        <div class="chapter-item" style="justify-content: center; cursor: default;">
            <span class="chapter-name">📷 ${manga.chapter_count} 張圖片</span>
        </div>
    `;
}
```

#### 無限滾動（已實現）
- 監聽滾動事件
- 距離底部 200px 時自動載入下一頁
- 每頁 6 個資料夾

### 3. 閱讀器優化 (reader.html - 已實現)

#### 順序載入模式
```javascript
loadAllImagesSequentially() {
    const loadNext = (index) => {
        if (index >= this.images.length) {
            // 全部載入完成
            return;
        }
        this.loadImageSequentially(index, loadNext);
    };
    
    // 從第一張開始
    loadNext(0);
}
```

#### 每張圖片獨立處理
```javascript
async loadImageSequentially(index, onComplete) {
    const state = this.imageStates.get(index);
    if (!state) return;
    
    state.loading = true;
    this.loadingQueue.add(index);
    
    const placeholder = document.getElementById(`placeholder-${index}`);
    // 顯示載入中動畫...
    
    const img = new Image();
    
    // 30秒超時
    const timeoutId = setTimeout(() => {
        this.handleImageErrorSequential(index, '載入超時', onComplete);
    }, 30000);
    
    img.onload = () => {
        clearTimeout(timeoutId);
        // 立即顯示圖片
        placeholder.replaceWith(img);
        state.loaded = true;
        state.loading = false;
        
        // 繼續載入下一張
        if (onComplete) onComplete(index + 1);
    };
    
    img.onerror = () => {
        clearTimeout(timeoutId);
        this.handleImageErrorSequential(index, '載入失敗', onComplete);
    };
    
    img.src = `/image/${encodeURIComponent(state.path)}?category=${this.category}&t=${Date.now()}`;
}
```

## 📊 Gallery 工作流程

### 列表頁面載入流程
```
1. 訪問 /gallery
   ↓
2. 載入前 6 個資料夾
   - 只掃描資料夾名稱
   - 快速統計圖片數量
   - 不載入任何圖片
   ↓
3. 顯示資料夾卡片
   - 資料夾名稱
   - "📷 X 張圖片"
   ↓
4. 滾動到底部時
   - 自動載入下 6 個資料夾
   - 無限滾動直到沒有更多
```

### 點擊進入閱讀器流程
```
1. 點擊 Gallery 資料夾
   ↓
2. 獲取資料夾詳情
   - API: /api/gallery/detail/{path}
   - 返回圖片列表
   ↓
3. 進入閱讀器頁面
   ↓
4. 順序載入圖片
   - 載入第 1 張 → 顯示 → 載入第 2 張 → 顯示 → ...
   - 每張圖片獨立處理
   - 載入完成立即顯示
   - 失敗自動重試（最多 3 次）
   ↓
5. 全部載入完成
```

## 🎯 性能優化效果

### 列表頁面
| 項目 | 優化前 | 優化後 |
|------|--------|--------|
| 初始載入 | 載入所有資料夾 | 只載入 6 個 |
| 圖片處理 | 掃描所有圖片 | 只計數，不載入 |
| 載入時間 | 可能 10-30 秒 | < 1 秒 |
| 內存使用 | 高（載入所有數據） | 低（按需載入） |

### 閱讀器頁面
| 項目 | 優化前 | 優化後 |
|------|--------|--------|
| 載入方式 | 滾動觸發懶加載 | 順序自動載入 |
| 顯示時機 | 需要滾動到位置 | 立即顯示 |
| 超時風險 | 高（一次載入多張） | 低（逐張載入） |
| 用戶體驗 | 需要等待和滾動 | 自動依序呈現 |

## 🔧 配置調整

### 調整每頁資料夾數量
在 `app.py` 中修改：
```python
@app.route('/api/gallery/list')
def get_gallery_list():
    per_page = request.args.get('per_page', 6, type=int)  # 改為其他數字
```

### 調整圖片載入超時
在 `reader.html` 中修改：
```javascript
const timeoutId = setTimeout(() => {
    this.handleImageErrorSequential(index, '載入超時', onComplete);
}, 30000);  // 改為其他毫秒數
```

### 調整自動重試次數
在 `reader.html` 的構造函數中：
```javascript
this.maxRetries = 3;  // 改為其他次數
```

## 📱 Gallery 專屬特性

### 資料夾結構
```
gallery/
├── 作者1/
│   ├── 001.jpg
│   ├── 002.jpg
│   └── 003.jpg
├── 作者2/
│   ├── 001.png
│   └── 002.png
└── 作品集/
    ├── image1.jpg
    └── image2.jpg
```

### 顯示方式
- 📷 **圖示**：表示 Gallery 作品
- **數字**：顯示圖片張數（不是章節數）
- **名稱**：使用資料夾名稱作為作品名稱

### 點擊行為
- 點擊資料夾 → 直接進入閱讀器（不需要選擇章節）
- 圖片依序載入顯示
- 沒有上一話/下一話（因為沒有章節概念）

## 🧪 測試建議

### 1. 測試列表載入
```
1. 訪問 http://localhost:5000/gallery
2. 確認只顯示 6 個資料夾
3. 滾動到底部
4. 確認自動載入更多資料夾
5. 檢查顯示 "📷 X 張圖片"
```

### 2. 測試進入閱讀器
```
1. 點擊任意 Gallery 資料夾
2. 觀察圖片是否依序載入
3. 確認每張圖片載入完成立即顯示
4. 不需要滾動即可看到所有圖片
```

### 3. 測試錯誤處理
```
1. 測試不存在的資料夾
2. 測試空資料夾
3. 測試損壞的圖片
4. 確認有友好的錯誤提示
```

## 📝 注意事項

1. **資料夾名稱**：支持中文和特殊字符
2. **圖片格式**：支持 `.jpg`, `.jpeg`, `.png`, `.gif`, `.webp`, `.bmp`, `.tiff`
3. **檔案大小**：建議單個圖片 < 10MB
4. **資料夾數量**：無限制，但建議合理組織
5. **網路速度**：較慢網路會增加每張圖片的載入時間

## 🎉 現在已完成

✅ Gallery 列表初始只載入 6 個資料夾  
✅ 滾輪捲動自動載入更多  
✅ 顯示圖片張數而不載入圖片  
✅ 閱讀器順序載入圖片  
✅ 每張載入完成立即顯示  
✅ 避免 timeout 問題  
✅ 自動重試機制  
✅ 友好的錯誤提示  

**Gallery 功能已全面優化完成！** 🎨
